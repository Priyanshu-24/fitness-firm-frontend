...
export default function Dashboard({history}) {
    ...
    const deleteEventHandler = async (eventId) => {
        ...
        try {
            ...
            setError(true)
            setMessageHandler('Error while deleting the event!');
            setTimeout(() => {
                setError(false)
                setMessageHandler('')
            }, 2000)
    ...
    const registrationRequestHandler = async (event) => {
        try {
            ...
            setSuccess(true)
            setMessageHandler(`The request for the event ${event.title} was successfull!`)
            setTimeout(() => {
                setSuccess(false)
                filterHandler(null)
                setMessageHandler('')
            ...
            setError(true)
            setMessageHandler(`The request for the event ${event.title} wasn't successfull!`)
            setTimeout(() => {
                setError(false)
                setMessageHandler('')
            }, 2000)
    ...
    return (
        ...
        <Dropdown isOpen={dropdownOpen} toggle={toggle}>
            ...
            <DropdownMenu>
                ...
                <DropdownItem onClick={() => filterHandler("badminton")} active={rSelected === 'badminton'} >Badminton</DropdownItem>
                <DropdownItem onClick={() => filterHandler("athletics")} active={rSelected === 'athletics'} >Athletics</DropdownItem>
                <DropdownItem onClick={() => filterHandler("tennis")} active={rSelected === 'tennis'} >Tennis</DropdownItem>
        ...
        <ul className = 'events-list'>
            {events.map(event => (
               ...
               <li key = {event._id}>
                  ...
                  <span>Sport Category : {event.sport}</span>
